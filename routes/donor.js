...
router.post("/signup_otp", async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL_NAME, // Use environment variable for the sender email address
      to: req.body.email,
      subject: "Login OTP",
    ...
    console.log("Email is sent to the mail id check it....");
...
router.get("/api/totalDonors", async (req, res) => {
  try {
    // const totalDonors = await Donor.countDocuments({
    //   last_donation_date: { $lte: new Date() },
    // });
    const totalDonors = await Donor.countDocuments();
